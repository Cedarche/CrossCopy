...
const FileDropzone = ({ mobile, setOpenFiles, files, setFiles, paidUser }) => {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: (acceptedFiles) => {
      ...
      const maxCumulativeSize = paidUser
        ? 2000 * 1024 * 1024
        : 500 * 1024 * 1024; // 500MB

      const tooBigFiles = acceptedFiles.filter(
                ...
                <AnimatePresence>
                  {files.map((file, index) => (
                    <FileI
                      ...
                      handleShareClick={handleOpenBottomSheet}
                      key={file.name}
                    />
                  ))}
                </AnimatePresence>
      ...
      {mobile && (
        <>
          <Heading style={{ marginLeft: "0px" }}>
            ...
            <ButtonContainer>
              {selectedFiles.length > 0 && (
                ...
                <Button onClick={handleOpenBottomSheet}>Share</Button>
              )}
              {selectedFiles.length > 0 && (
                <Button onClick={downloadFiles}>Download</Button>
              )}
          ...
          <Sheet
            ...
            snapPoints={[500,  0]}
          >
            <Sheet.Container style={{backgroundColor: '#3b3b3b'}}>
              <Sheet.Header />
              <Sheet.Content style={{display: 'flex', width: '100%', flex: 1, alignItems: 'center', justifyContent:'center', border: '1px solid'}}>
