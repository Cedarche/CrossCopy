...
import { handleSharePress, getShareLink } from "./ShareFunctions";
  ...
  return (
    ...
    >
      ...
      {index === 0 ? (
        ...
        <ShareOptions file={selectedFile} />
      )}
    </BottomSheetModal>
  );
};
...
const Options = ({ handleShare, handleRemoveFile, file }) => {
  return (
    <View style={styles.subContainer}>
      ...
      <TouchableOpacity style={styles.optionRow} onPress={() => handleRemoveFile(file)}>
...
const ShareOptions = ({ file }) => {
  const [shareLink, setShareLink] = useState(null);

  //   const { shareLink } = getShareLink(file);
  //   console.log(shareLink)

  useEffect(() => {
    async function getLink() {
      console.log(file);
      const shareLink = await getShareLink([file]);
  ...
  return (
    <View style={[styles.shareSubContainer]}>
      <TouchableOpacity
        ...
        onPress={() => handleSharePress(shareLink)}
      >
        {shareLink ? (
          <QRCode
            value={shareLink}
        ...
        <Text style={{ fontSize: 16, color: "#bebebe", marginTop: 20 }}>
