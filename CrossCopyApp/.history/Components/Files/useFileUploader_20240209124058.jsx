...
const useFileUploader = (storage, database, auth) => {
  ...
  const uploadFileToFirebase = async (fileUri, fileName, fileSize) => {
    ...
    const fileStorageRef = storageRef(storage, `uploads/${auth.currentUser.uid}/${sanitizedFileName}`);
    ...
    try {
      ...
      uploadTask.on('state_changed', 
        (snapshot) => {
          ...
          setUploadProgress(prev => ({ ...prev, [sanitizedFileName]: progress }));
        }, 
        (error) => {
          setError(error);
        ...
        async () => {
          ...
          const fileDbRef = dbRef(database, `uploads/${auth.currentUser.uid}/${sanitizedFileName}`);
