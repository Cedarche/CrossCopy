...
export default function Files({ files, setFiles, paidUser }) {
  ...
  const { uploadFileToFirebase, uploadProgress, error } = useFileUploader(storage, database, auth);
  ...
  const pickImages = () => {
    ImagePicker.openPicker({
      multiple: true,
      mediaType: "photo", // Specify 'photo' to avoid picking videos
      compressImageQuality: 0.8, // Compress image quality
      forceJpg: true, // Force convert to JPG
    }).then(images => {
      images.forEach(image => {
        // The image.path now should point to a JPG version of the selected image
        const fileUri = image.path;
        const fileName = `${image.filename || fileUri.split('/').pop().split('.')[0]}.jpg`; // Change the extension to .jpg
        const fileSize = image.size; // This might not reflect the size of the converted JPG if the original was a HEIC
  
        // Upload the converted image to Firebase
        uploadFileToFirebase(fileUri, fileName, fileSize);
      });
    }).catch(error => {
